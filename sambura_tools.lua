...
function DrawNewMenu()
    ...
    imgui.Checkbox(u8'РЎР±РёРІ (H)', GUI.main.Sbiv)
    imgui.Checkbox('Instant Cross', GUI.main.Crosshair)
    if imgui.Button(u8'РљСЂР°С€ Р�РіСЂС‹', imgui.ImVec2(280, 0)) then
        Crash()
    end
    if imgui.Button(u8'Р—Р°РєСЂС‹С‚СЊ РњРµРЅСЋ', imgui.ImVec2(280, 0)) then
    ...
    if GUI.main.Radius.v then
        for IDTEXT = 0, 2048 do
            if sampIs3dTextDefined(IDTEXT) then
                ...
                if text == u8"РЈРїСЂР°РІР»РµРЅРёСЏ С‚РѕРІР°СЂР°РјРё." and not isCentralMarket(posX, posY) then
...
function sampev.onSetObjectMaterialText(ev, data)
    ...
    if doesObjectExist(Object) and getObjectModel(Object) == 18663 and string.find(data.text, u8"(.-) {30A332}РЎРІРѕР±РѕРґРЅР°СЏ!") then
