                    ...
                    if k then 
                        ...
                        if updateversion~=thisScript().version then 
                            lua_thread.create(function(b)
                                local d=require('moonloader').download_status;
                                local m=-1;
                                sampAddChatMessage(b..'Обнаружено обновление. Пытаюсь обновиться с '..thisScript().version..' на '..updateversion,m)
                                wait(250)
...
function DrawMainMenu()
    ...
    imgui.Checkbox('Дрифт Мастер (Shift)', GUI.main.Drift)
